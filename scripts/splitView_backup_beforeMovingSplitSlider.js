...
// add function to the screen split slider between 2 maps
function onDrag(e) {
    const containerWidth = document.body.clientWidth; // Total screen width
    const sliderPosition = Math.max(0, Math.min(e.clientX, containerWidth)); // Keep slider within bounds

    // Adjust slider position
    splitSlider.style.left = sliderPosition + 'px';

    // Adjust map widths dynamically
    const map1Width = (sliderPosition / containerWidth) * 100;
    const map2Width = 100 - map1Width;

    container1.style.width = map1Width + '%';
    container2.style.width = map2Width + '%';

    // Tell Leaflet to re-render the maps
    map.invalidateSize();
    map2.invalidateSize();
}
...
document.getElementById('split-view-btn').addEventListener('click', () => {
    if (container1.classList.contains('split-map')) {
        ...
        //// change the year slider 2 to split view styling mode
        slider2.classList.remove('hidden');
        slider2.classList.add('split-year-slider2');

        //// change the screen slider to split view styling mode
        splitSlider.classList.remove('hidden');
        splitSlider.classList.add('split-screen-slider');
        ...
        splitSlider.addEventListener('mousedown', function (e) {
